...
const filtraVeicoli = computed(() => {
  let result = veicoli.value.filter(v => v.tipo === 'noleggio')
  ...
  <section class="bg-black text-white py-10 px-6 min-h-screen grid grid-cols-1 md:grid-cols-[280px_1fr] gap-8">
    ...
    <main class="space-y-6 w-full">
      <div class="flex flex-wrap justify-between items-center gap-4">
        ...
        <!-- Vista -->
        <div class="flex items-center gap-2">
          <button @click="vista = 'griglia'" :class="vista === 'griglia' ? 'bg-white/10 border border-white text-red-500' : 'text-white/40'" class="p-2 rounded transition hover:bg-white/10">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M3 3h7v7H3V3zm0 11h7v7H3v-7zm11-11h7v7h-7V3zm0 11h7v7h-7v-7z"/>
            </svg>
          </button>
          <button @click="vista = 'lista'" :class="vista === 'lista' ? 'bg-white/10 border border-white text-red-500' : 'text-white/40'" class="p-2 rounded transition hover:bg-white/10">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div v-else class="space-y-4">
        <div v-for="v in filtraVeicoli" :key="v.codice" class="bg-[#111] p-4 rounded-lg border border-white/10 flex flex-col sm:flex-row gap-4">
          ...
          <div class="flex-1 space-y-1">
            ...
            <p class="text-xs text-white/50">Venditore: {{ v.venditore }}</p>
            <NuxtLink :to="`/noleggio/${slugify(v.titolo)}-${v.codice}`" class="inline-block mt-2 bg-red-700 hover:bg-red-800 text-white px-4 py-1.5 text-sm rounded shadow">Dettagli</NuxtLink>
          </div>
        </div>
      </div>
